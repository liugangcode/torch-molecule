...
class DeFoGMolecularGenerator(BaseMolecularGenerator):
    ...
    transition : str, default='marginal'
        Type of transition for flow matching ('marginal' or other options)
    time_distortion : str, default="polydec"
        Type of time distortion for the flow process
    lambda_train : List[float], default=[5.0, 1.0]
        Loss weights for edge loss (lambda_train[0]) and property loss (lambda_train[1])
    extra_features_type : str, default='rrwp'
        Type of extra features to compute ('rrwp' for random walk positional encoding)
    rrwp_steps : int, default=16
        Number of steps for random walk positional encoding
    sample_steps : int, default=500
        Number of sampling steps during generation
    rdb : str, default="general"
        Rate matrix design method
    rdb_crit : str, default="p_x1_g_xt"
        Rate matrix design criterion
    eta : float, default=0
        Eta parameter for rate matrix design
    omega : float, default=0.1
        Omega parameter for rate matrix design
    guidance_weight : float, default=0.2
        Weight for classifier-free guidance during sampling
    ...
    # Model parameters from GraphTransformer
    num_layer: int = 6
    hidden_mlp_dims: Dict[str, int] = field(default_factory=lambda: {'X': 256, 'E': 128, 'y': 128})
    hidden_dims: Dict[str, Any] = field(default_factory=lambda: {'dx': 256, 'de': 64, 'dy': 64, 'n_head': 8, 'dim_ffX': 256, 'dim_ffE': 128, 'dim_ffy': 128})
    
    # Flow Matching & Training parameters from DeFoG
    transition: str = 'marginal'
    time_distortion: str = "polydec"
    lambda_train: List[float] = field(default_factory=lambda: [5.0, 1.0])
    extra_features_type: str = 'rrwp'
    rrwp_steps: int = 16

    # Sampling parameters from DeFoG
    ...
    # Standard training parameters from torch-molecule, mirroring GraphDIT
    batch_size: int = 128
    epochs: int = 1000
    ...
    task_type: List[str] = field(default_factory=list) # For compatibility
    ...
    def __post_init__(self):
        ...
        self.is_conditional = len(self.task_type) > 0
    ...
    def _get_param_names() -> List[str]:
        ...
        return [
            ...
            "input_dim_X", "input_dim_E", "input_dim_y", "task_type", "is_conditional",
    ...
    def generate(self, labels: Optional[Union[List[List], np.ndarray, torch.Tensor]] = None, num_nodes: Optional[Union[List[List], np.ndarray, torch.Tensor]] = None, batch_size: int = 32) -> List[str]:
        ...
        is_conditional = len(self.task_type) > 0
        if is_conditional and labels is None:
